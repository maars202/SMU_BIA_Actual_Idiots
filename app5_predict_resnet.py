...
def app():
    ...
    def create_resnet_s():
        resnet50 = tf.keras.applications.ResNet50V2(
            ...
            include_top = False, # Include top gives a classification layer for 1000 classes, we don't need that
            weights = 'imagenet', 
            pooling = 'avg' # Ensures that output of pre-trained model is 1-dimensional, output is single vector
        )

        resnet50.trainable = False # Don't change the original weights

        x = layers.Dense(500, activation = 'relu')(resnet50.output)
        x = layers.Dense(250, activation = 'relu')(x)
        outputs = layers.Dense(10, activation = 'softmax')(x)
        # outputs = layers.Dense(10, activation = 'softmax')(resnet50.output)
